...
public class MoeaddIntegerSolution<S extends IntegerSolution> extends AbstractMOEAD<S> {

	private Logger logger = Logger.getLogger(MoeaddIntegerSolution.class);
	...
	private List<DagSchedulingPopulationOfGeneration<S>> popOfGenList;
	...
	public MoeaddIntegerSolution(Problem<S> problem, int populationSize, int resultPopulationSize,
			...
			double neighborhoodSelectionProbability, int maximumNumberOfReplacedSolutions, int neighborSize, DagSchedulingConfiguration config) {
		...
		this.config = config;		
		popOfGenList = new ArrayList<DagSchedulingPopulationOfGeneration<S>>();
		this.thirdObjectiveName = this.config.getThirdObjectiveName();

		logger.debug("");
		logger.info("Algorithm: " + getName());
		logger.info("Population Size: " + this.populationSize);
		logger.info("Max Generation : " + this.maxEvaluations);
		logger.debug("");
	}
	...
	public void run() {
		...
		do {
			...
			// Collect the solution each iteration

			DagSchedulingPopulationOfGeneration<S> pop = new DagSchedulingPopulationOfGeneration<S>();
			...
			String generationLog = String.format("[%s, %s, %d]: add Gen %d of %d",
					this.config.getAlgorithmType().getName(), this.config.getClusterType().getName(),
	...
	protected void initializeUniformWeight() {
		if ((problem.getNumberOfObjectives() == 2) && (populationSize <= 300)) {
			...
			try {
				...
				InputStream in = new FileInputStream(dataDirectory + "/" + dataFileName);
